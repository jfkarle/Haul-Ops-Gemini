...
class BoatHaulingManager:
    ...
    def add_customer(self):
        ...
        while True:
            try:
                boat_length = float(input("Boat Length (ft): "))
                break
            except ValueError:
                print("Invalid length. Please enter a number.")
        boat_name = input("Boat Name (optional): ")
    ...
    def find_customer_interactive(self):
        ...
        if not results:
            ...
            while True:
                try:
                    choice = int(input("Select customer number: ")) - 1
    ...
    def add_job(self):
        ...
        while True:
            scheduled_datetime_str = input(f"Scheduled Date and Time (YYYY-MM-DD HH:MM, e.g., {datetime.now().strftime('%Y-%m-%d %H:%M')}): ")
            try:
                # Validate format before creating Job object
                datetime.strptime(scheduled_datetime_str, DATETIME_FORMAT)
                break
            except ValueError:
                print(f"Invalid datetime format. Please use YYYY-MM-DD HH:MM")
        ...
        while True:
            try:
                quoted_price = float(input("Quoted Price ($): "))
                break
            except ValueError:
...
def main_menu(manager):
    while True:
        print("\n--- Boat Hauling Business Automator ---")
        print("Current Time:", datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
        print("\n-- Customers --")
        print("1. Add New Customer")
        print("2. List/View All Customers")
        print("3. Find Customer")
        print("\n-- Jobs --")
        print("4. Add New Job")
        print("5. List/View Jobs (by status or all)")
        print("6. Update Job Status")
        print("\n-- System --")
